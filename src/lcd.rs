...
impl MuMatrix {
    ...
    /// 反显端口字母 (A/B): 整个 5x8 格位填满 (set=1→黑), 中间 3x5 用窄体字母镂空 (0→绿), 从第 2 行 (y+1) 开始
    /// 依据 John 权威: "先用 5x8 全亮(黑), 中间 3x5 显示 A/B, 且从第二行开始"
...
mod tests {
    ...
    fn programmatic_part_decode() {
        ...
        // A 是反显 (3x5, 从第 2 行 y1 开始): 字形区 x11..13,y1..5 镂空; 其余 (边框列 x10/x14, 及 y0/y6/y7 行) 全黑(set=1)
        // 边框列 x10 x14 (0..7 行) 全黑
        for y in 0..8 { for x in [10i32, 14] {
            assert_eq!(pm.get(x, y), 1, "反显边框列应黑(1), at ({x},{y})");
        }}
        // 行 y0 / y6 / y7 (全 5 列) 全黑
        for y in [0i32, 6, 7] { for x in 10..15 {
